...
public:
  ...
  void options_description(boost::program_options::options_description& desc) const override
  {
    ...
    desc.add_options()
      ...
      ("env,e",         boost::program_options::value<vector<string>>(), "environment variables and files")
  ...
  int run() override
  {
    ...
    runtime_path = "/var/lib/" + configuration.variable("name");
...
private:
  ...
  bool run_deploy_command()
  {
    ...
    command << " -p \"" << tarball << '"'
      ...
      << " --start \"" << install_root << '/' << configuration.variable("name") << "/start.sh\""
      << " --stop \"" << install_root << '/' << configuration.variable("name") << "/stop.sh\"";
    if (options.count("port"))
      command << " --app-port " << options["port"].as<unsigned short>();
    ...
    {
      command << " --env";
      for (const auto& var : options["env"].as<std::vector<std::string>>()) command << ' ' << var;
    }
    if (options.count("verbose"))
      std::cout << "+ " << command.str() << std::endl;
    return Crails::run_command(command.str());
