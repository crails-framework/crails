...
class DockerBuildOdbCompiler : public Crails::Template
{
public:
  DockerBuildOdbCompiler(const Crails::Renderer* renderer, Crails::SharedVars& vars) :
...
std::string render_docker_build_odb_compiler(const Crails::Renderer* renderer, Crails::SharedVars& vars)
{
  return DockerBuildOdbCompiler(renderer, vars).render();
}